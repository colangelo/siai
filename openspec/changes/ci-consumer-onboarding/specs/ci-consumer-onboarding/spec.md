...
The documented, repeatable process for onboarding a real (non-demo) repository to the siai homelab CI/CD stack — forge repository, Harbor project + robot scope, Woodpecker activation/trust/secrets, and a tag-gated build-push to Harbor — with `ac/direction` as the reference implementation.
